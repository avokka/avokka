...
package object avokka {
  ...
  implicit final class ApplicativeDecoderOps[T](private val decoder: Decoder[T]) {
    def decodeA[F[_]](bits: BitVector)(implicit A: ApplicativeError[F, Throwable]): F[DecodeResult[T]] = A.fromEither(attemptEither(decoder.decode(bits)))
  }

  implicit final class ApplicativeEncoderOps[T](private val encoder: Encoder[T]) {
    def encodeA[F[_]](value: T)(implicit A: ApplicativeError[F, Throwable]): F[BitVector] = A.fromEither(attemptEither(encoder.encode(value)))
  }
}
