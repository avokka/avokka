...
object CollectionRevision { self =>
  ...
  implicit val api: Api.EmptyBody.Aux[Collection, CollectionRevision.type, Response] =
    new Api.EmptyBody[Collection, CollectionRevision.type] {
      override type Response = self.Response
      override def header(collection: Collection,
                          command: CollectionRevision.type): Request.HeaderTrait = Request.Header(
