...
object DocumentPatch {
  ...
  implicit def api[P : VPackEncoder, T : VPackDecoder]: Api.Aux[Database, DocumentPatch[P, T], P, Document.Response[T]] = new Api[Database, DocumentPatch[P, T], P] {
    override type Response = Document.Response[T]
    override def header(database: Database, command: DocumentPatch[P, T]): Request.HeaderTrait = Request.Header(
    ...
    override def body(context: Database, command: DocumentPatch[P, T]): P = command.patch
