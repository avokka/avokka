...
class VPackDoubleCodecSpec extends FlatSpec with Matchers with ScalaCheckDrivenPropertyChecks with VPackCodecSpecTrait {
  ...
  it should "encode at 0x1b" in {
    forAll { d: Double =>
      ...
      assertCodec(VPackDoubleCodec, VPackDouble(d),
        hex"1b" ++ codecs.ulongBytes(lbits, 8).bytes
      )
    }

    assertCodec(VPackDoubleCodec, VPackDouble(1.5d), hex"1b 000000000000F83F")
    assertCodec(VPackDoubleCodec, VPackDouble(-1.5d), hex"1b 000000000000F8BF")
    assertCodec(VPackDoubleCodec, VPackDouble(1.23456789d), hex"1b 1B DE 83 42 CA C0 F3 3F")
    assertCodec(VPackDoubleCodec, VPackDouble(0.001d), hex"1bfca9f1d24d62503f")
  ...
  "codec" should "fail if head is not a long" in {
    assert(VPackDoubleCodec.decode(hex"00".bits).isFailure)
  }
}
